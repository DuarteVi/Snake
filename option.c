...
int tailleMax = 300;											/* Taille maximale du serpentq*/
...
int taille_serpent = 10;										/* Taille du serpent affiché sur le côté */
...
void snake() {
		...
		CopierZone(5,0,0,0,1250,600,0,0);
...
int option(int *Param) {
	ChoisirEcran(5);
	couleur cc1=CouleurParComposante(20,120,20);
    ...
    int nbr_obstacle = 0;										/* Valeur par défaut */
    ...
    while (True) {
	   ...
	   /* --------------- On initialise les onglets ---------------*/
	    ...
	    CopierZone(2,5,0,0,longueur*10,hauteur*10,590,100);				/* Pour afficher le terrain */
	    ...
	    if (SourisCliquee()) {
			...
			*/
	    	...
	    	if (nbr_obstacle_select)
	    	{
		    	...
		    	if ( _X > 435 && _X < 460 && _Y > 180 && _Y < 205) {
	  				nbr_obstacle = 0;
		    	}
